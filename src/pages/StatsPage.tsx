...
import {
  ...
  Radar, RadarChart, PolarGrid, PolarAngleAxis, PieChart, Pie, Cell
} from 'recharts';
import { Trophy, Flame, Clock, Target, Brain, PieChart as PieIcon } from 'lucide-react';
...
export function StatsPage() {
  ...
  const statsSummary = useMemo(() => [
    ...
    { label: "专注总时���", value: `${(totalFocusMinutes / 60).toFixed(1)}h`, icon: Clock, color: "text-blue-500", bg: "bg-blue-50" },
  ...
  const distributionData = useMemo(() => {
    ...
    tasks.forEach(t => { if (t.status === 2) counts[t.type] += t.pomodoroSpent * 25; });
  ...
  return (
    <AppLayout container>
      <header className="mb-12">
        ...
        <p className="text-muted-foreground text-lg mt-2 font-medium">洞察你的���筑规律与成长轨迹</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
        <Card className="lg:col-span-8 border-none shadow-soft rounded-[3rem] p-8 bg-card">
          <CardHeader className="px-0 pt-0 pb-8"><CardTitle className="text-2xl font-display font-bold text-foreground">近七日专注趋���</CardTitle></CardHeader>
