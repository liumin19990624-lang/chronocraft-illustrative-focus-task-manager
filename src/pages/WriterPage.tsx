...
import { PenTool, Sparkles, Zap, Languages, SearchCode, Loader2, Brain, FileText } from 'lucide-react';
...
export function WriterPage() {
  ...
  const handleAiAction = async (type: 'modify' | 'evaluate') => {
    if (!text.trim()) return toast.warning("���先挥毫泼墨");
    setAiPanelOpen(true);
    setIsAiProcessing(true);
    const result = await requestAi(text, type);
    setAiResult(result);
    setIsAiProcessing(false);
    setSpirit(prev => Math.max(0, prev - 20));
  };
  return (
    <AppLayout container>
      <div className="max-w-7xl mx-auto">
        <header className="mb-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-lg">笔灵助阵的学术写作��间</p>
          </div>
          ...
          <aside className="lg:col-span-4 space-y-8">
            <Card className="p-8 rounded-[2.5rem] bg-card/60 backdrop-blur-xl border-none shadow-soft">
              ...
              <div className="flex flex-col items-center justify-center py-20 opacity-30 text-center">
                ...
                <p className="text-lg font-bold">挥毫书写，笔灵方显</p>
              </div>
            </Card>
          </aside>
        </div>
